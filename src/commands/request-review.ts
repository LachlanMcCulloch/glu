...
export async function requestReview(range: string, options: RequestReviewOptions) {
  try {
    const git = simpleGit();
    ...
    if (range.includes('-')) {
      ...
      if (isNaN(start) || isNaN(end)) {
        console.error('Invalid range format. Use "n" or "n-m" where n and m are numbers.');
        process.exit(1);
      }
    ...
    // Validate range
    if (startIndex < 0 || endIndex >= allCommits.length || startIndex > endIndex) {
      console.error(`Invalid range ${range}. Available commits: 1-${allCommits.length}`);
      process.exit(1);
    }
    ...
    try {
      ...
      if (options.force) {
        ...
        console.error(`Branch ${targetBranch} already exists. Use --force to overwrite.`);
        process.exit(1);
      }
    } catch {
    ...
    console.log(`Cherry-picking ${selectedCommits.length} commit(s) from range ${range}...`);
    ...
    if (options.push) {
      ...
      if (options.force) {
        ...
        try {
          ...
          console.log('Use --force to force push if the branch exists on origin');
          process.exit(1);
        }
      }
    } else {
